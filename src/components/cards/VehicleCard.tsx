...
export function VehicleCard({ vehicle }: { vehicle: VehicleType }) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-border bg-white p-6 shadow-[var(--shadow-premium)] transition-all duration-300 ease-[var(--ease-premium)] hover:-translate-y-1.5 hover:border-primary-200 hover:shadow-[var(--shadow-premium-lg)]"
    >
      <div
      ...
      <div className="relative z-10">
        ...
        <h3 className="mt-5 text-lg font-bold text-ink-900">{vehicle.name}</h3>
        <p className="mt-1 text-sm text-ink-500">{vehicle.category}</p>
      </div>
